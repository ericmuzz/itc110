...
class Projectile:
    ...
    def __init__(self, angle=None, speed=None, initial_height=1):
        ...
        if speed is None:
            raise Exception('Must set initial speed.')
        else:
            self.speed = speed
    ...
    def get_distance(self):
        while self.y >= 0:
            ...
            new_y_velocity = self.y_velocity - (self.time_interval * 9.8)
            self.y = self.y + (self.time_interval * ((self.y_velocity + new_y_velocity)/2))
            self.y_velocity = new_y_velocity
        return self.x

class PumpkinProject(Projectile):
    speed = 40

    def __init__(self, *args, **kwargs):
        kwargs['speed'] = self.speed
        super(PumpkinProject, self).__init__(*args, **kwargs)

class AppleProject(Projectile):
    speed = 200

    def __init__(self, *args, **kwargs):
        kwargs['speed'] = self.speed
        super(AppleProject, self).__init__(*args, **kwargs)

print('We want to put the observation stands at 500m.')

humpty = PumpkinProject(angle=30)
print('Pumpkin humpty will make it to', humpty.get_distance(), 'meters.')

dumpty = PumpkinProject(angle=50, initial_height=2)
print('Pumpkin dumpty will make it to', dumpty.get_distance(), 'meters.')

johnny = AppleProject(angle=45)
